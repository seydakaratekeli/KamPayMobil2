...
namespace KamPay.ViewModels
{
    ...
    public partial class ProductDetailViewModel : ObservableObject
    {
        ...
        [RelayCommand]
        private async Task SimulatePurchaseAsync()
        {
            if (Product == null)
            {
                await Shell.Current.DisplayAlert("Hata", "Ürün bilgisi bulunamadı.", "Tamam");
                return;
            }
            ...
            var confirm = await Shell.Current.DisplayAlert(
                "Satın Al",
                $"{Product.Title} adlı ürünü {Product.Price} ₺ karşılığında satın almak istiyor musunuz?",
                "Evet",
                "Hayır"
            );
            ...
            if (currentUser == null)
            {
                await Shell.Current.DisplayAlert("Hata", "Giriş yapılmamış.", "Tamam");
                return;
            }

            try
            {
                // 1️⃣ Ödeme simülasyonunu başlat
                var createResult = await _transactionService.CreatePaymentSimulationAsync(Product.ProductId, "CardSim");
                if (!createResult.Success)
                {
                    await Shell.Current.DisplayAlert("Hata", createResult.Message, "Tamam");
                    return;
                }

                var payment = createResult.Data;

                // 2️⃣ OTP doğrulaması (kart ödemesi için)
                if (payment.Method == PaymentMethodType.CardSim)
                {
                    var otp = await Shell.Current.DisplayPromptAsync(
                        "OTP Doğrulaması",
                    ...
                    {
                        await Shell.Current.DisplayAlert("Uyarı", "OTP girişi iptal edildi.", "Tamam");
                        return;
                    }

                    var confirmResult = await _transactionService.ConfirmPaymentSimulationAsync(Product.ProductId, payment.PaymentId, otp);
                    if (!confirmResult.Success)
                    {
                        await Shell.Current.DisplayAlert("Hata", confirmResult.Message, "Tamam");
                        return;
                    }
                }

                // 3️⃣ EFT simülasyonu (doğrudan onaylanır)
                else if (payment.Method == PaymentMethodType.BankTransferSim)
                {
                    await Shell.Current.DisplayAlert(
                        "Bilgi",
                        $"Lütfen {payment.BankName} hesabına {payment.Amount} ₺ gönderin.\nReferans No: {payment.BankReference}",
                        "Tamam"
                    );

                    // Demo: EFT'nin onaylanması birkaç saniye sürebilir
                    await Task.Delay(2000);
                    var confirmResult = await _transactionService.ConfirmPaymentSimulationAsync(Product.ProductId, payment.PaymentId);
                    if (!confirmResult.Success)
                    {
                        await Shell.Current.DisplayAlert("Hata", confirmResult.Message, "Tamam");
                        return;
                ...
                // 4️⃣ Başarılı işlem sonrası bildirim
                await Shell.Current.DisplayAlert(
                    "Başarılı",
                    "Ödeme başarıyla tamamlandı ve ürün size ait!",
                    "Tamam"
                );

                // 5️⃣ Ürünü satıldı olarak güncelle
                await _productService.MarkAsSoldAsync(Product.ProductId);
            ...
            {
                await Shell.Current.DisplayAlert("Hata", $"Ödeme simülasyonu başarısız: {ex.Message}", "Tamam");
            }
        }
